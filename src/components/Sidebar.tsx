...
export default function Sidebar() {
  ...
  useEffect(() => {
    if (!user) return;
    getContractsExpiringWithin(user, 30).then(contracts => {
      setUrgentCount(contracts.length);
    }).catch(() => setUrgentCount(0));
  }, [user]);

  // Navigation definition — adminOnly items only appear for admin role
  const allNavItems = [
    ...
    { label: "Alerts Center", href: "/alerts", icon: BellRing, badge: urgentCount },
  ...
  return (
    <>
      <Button
        variant="outline"
        size="icon"
        className="md:hidden fixed top-3 left-4 z-50 bg-white shadow-sm"
      ...
      <div className={cn(
        ...
        {/* User footer */}
        {user && (
          <div className="p-4 border-t border-slate-700/50 bg-slate-800/30 shrink-0">
