...
export default function Header() {
  ...
  // Only contracts visible to the logged-in user, expiring within 30 days
  const recentAlerts = getVisibleContractsExpiringWithin(user, 30)
    ...
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6 sticky top-0 z-20 shadow-sm ml-12 md:ml-0">
      <div className="flex items-center w-full max-w-md">
        <div className="relative w-full">
          ...
          <Input
            type="search"
            placeholder="Search academies or contracts..."
      ...
      <div className="flex items-center space-x-3 md:space-x-4 ml-auto">
        ...
        {/* Add Contract — visible to all authenticated users */}
              ...
              <>
                ...
                >
                  <div className="p-3 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <span className="font-semibold text-sm text-slate-900">Contract Alerts</span>
            ...
            {showUserMenu && (
              <>
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10, scale: 0.95 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 mt-2 w-52 bg-white rounded-lg shadow-lg border border-slate-200 z-50 overflow-hidden py-1"
                >
                  <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
                    <div className="text-sm font-bold text-slate-900">{user?.name}</div>
                    <div className="text-xs text-slate-500 font-mono">{user?.employeeId}</div>
                  </div>
                  <div className="py-1">
                    <div className="flex items-center px-4 py-2 text-sm text-slate-500 cursor-default">
